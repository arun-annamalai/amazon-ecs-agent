...
const (
	...
	servicesRegistryKeyRoot     = `System\CurrentControlSet\Services`
	taskCredentialsRegistryKey  = `AmazonECSCCGPlugin3`
	RegKeyPathFormat            = `HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\AmazonECSCCGPlugin3\%s`
)
...
func (cs *CredentialSpecResource) Create() error {
	...
	if ok {
		...
		seelog.Errorf("iamCredentials set successfully")
	} else {
		seelog.Errorf("NO iamCredentials")
	}
	...
	if domainlessGMSATask {
		...
		seelog.Errorf("Set credentials successfully for windowsregistrykey")
	}
...
func (cs *CredentialSpecResource) handleCredentialspecFile(credentialspec string) error {
	...
	if strings.HasPrefix(credentialspec, "credentialspecdomainless:") {
		// We need to copy the customers local file in order to avoid modifying their credentialspec
		credSpecFile, err = cs.copyCredentialSpecLocalFile(credentialspec)
		if err != nil {
			return err
		}

		// Fill in appropriate domainless gMSA fields
		err = fillInDomainlessFields(filepath.Join(cs.credentialSpecResourceLocation, credSpecFile), cs.taskARN)
		if err != nil {
			cs.setTerminalReason(err.Error())
			return err
		}
...
func (cs *CredentialSpecResource) handleS3CredentialspecFile(originalCredentialspec, credentialspecS3ARN string, iamCredentials credentials.IAMRoleCredentials) error {
	...
	err = handleDomainlessGMSACredSpec(originalCredentialspec, localCredSpecFilePath, cs.taskARN)
	if err != nil {
		cs.setTerminalReason(err.Error())
		return err
	}
...
func (cs *CredentialSpecResource) deleteTaskExecutionCredentialsRegKeys() error {
	...
	pathToTaskCredentialsRegistryKey := servicesRegistryKeyRoot + "\\" + taskCredentialsRegistryKey
	k, err := registry.OpenKey(registry.LOCAL_MACHINE, pathToTaskCredentialsRegistryKey, registry.ALL_ACCESS)
	if err != nil {
		seelog.Errorf("Error opening %s key to delete: %s", pathToTaskCredentialsRegistryKey, err)
		return err
	}
	defer k.Close()
	...
	if err != nil {
		seelog.Errorf("Error deleting %s key: %s", pathToTaskCredentialsRegistryKey+"\\"+cs.taskARN, err)
	}
	return nil
}
...
// This function makes a copy of the local credential spec file to avoid modifying customer resources
// Example Usage:
// copyCredentialSpecLocalFile("credentialspecdomainless:file://gmsa-cred.json") - single file
// copyCredentialSpecLocalFile("credentialspecdomainless:file://gmsa-dir\gmsa-cred.json") - Relative path with Windows file path separators
func (cs *CredentialSpecResource) copyCredentialSpecLocalFile(credentialspec string) (string, error) {
	relativeFilePath := strings.TrimPrefix(credentialspec, "credentialspecdomainless:file://")
	dir, originalFileName := filepath.Split(relativeFilePath)

	// Generate unique filename
	taskId, err := getTaskIDfromARN(cs.taskARN)
	if err != nil {
		return "", err
	}
	containerName := cs.credentialSpecContainerMap[credentialspec]
	copiedFileName := fmt.Sprintf("%s_%s_%s", taskId, containerName, originalFileName)

	// Copy new file in same directory
	// Read all the contents of the original file
	bytes, err := os.ReadFile(filepath.Join(cs.credentialSpecResourceLocation, dir, originalFileName))
	if err != nil {
		return "", err
	}

	// Copy all the contents to the destination file
	err = os.WriteFile(filepath.Join(cs.credentialSpecResourceLocation, dir, copiedFileName), bytes, 0777)
	if err != nil {
		return "", err
	}
...
func setTaskExecutionCredentialsRegKeys(taskCredentials credentials.IAMRoleCredentials, taskArn string) error {
	...
	seelog.Errorf("SetTaskExecutionCredentialsRegKey")
	k, err := registry.OpenKey(registry.LOCAL_MACHINE, servicesRegistryKeyRoot, registry.WRITE)
	if err != nil {
		seelog.Errorf("Error opening %s key: %s", servicesRegistryKeyRoot, err)
		return err
	}
	defer k.Close()

	k2, _, err := registry.CreateKey(k, taskCredentialsRegistryKey, registry.WRITE)
	if err != nil {
		seelog.Errorf("Error creating %s key: %s", taskCredentialsRegistryKey, err)
		return err
	}
	defer k2.Close()

	k3, _, err := registry.CreateKey(k2, taskArn, registry.WRITE)
	if err != nil {
	...
	err = k3.SetStringValue("AKID", taskCredentials.AccessKeyID)
	if err != nil {
		seelog.Errorf("Error creating AKID child value for task %s:%s", taskArn, err)
		return err
	}
	err = k3.SetStringValue("SKID", taskCredentials.SecretAccessKey)
	if err != nil {
		seelog.Errorf("Error creating SKID child value for task %s:%s", taskArn, err)
		return err
	}
	err = k3.SetStringValue("SESSIONTOKEN", taskCredentials.SessionToken)
	if err != nil {
...
func handleDomainlessGMSACredSpec(originalCredSpec, localCredSpecFilePath, taskARN string) error {
	// Exit early for non domainless gMSA cred specs
	if !strings.HasPrefix(originalCredSpec, "credentialspecdomainless:") {
		return nil
	}

	err := fillInDomainlessFields(localCredSpecFilePath, taskARN)
	if err != nil {
		return err
	}
	return nil
...
func fillInDomainlessFields(filePath, taskARN string) error {
	...
	if activeDirectoryConfigUntyped, ok := credSpec["ActiveDirectoryConfig"]; ok {
		activeDirectoryConfig := activeDirectoryConfigUntyped.(map[string]interface{})
		if hostAccountConfigUntyped, ok2 := activeDirectoryConfig["HostAccountConfig"]; ok2 {
			hostAccountConfig := hostAccountConfigUntyped.(map[string]interface{})
			if pluginInputString, ok3 := hostAccountConfig["PluginInput"]; ok3 {
				var pluginInputParsed PluginInput
				pluginInputString2 := pluginInputString.(string)
				err = json.Unmarshal([]byte(pluginInputString2), &pluginInputParsed)
				if err != nil {
					return err
				}
...
func getTaskIDfromARN(taskARN string) (string, error) {
	...
	return taskArnSplit[length-1], nil
}
