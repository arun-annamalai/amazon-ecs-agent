...
func TestClearCredentialSpecData(t *testing.T) {
	testCases := []struct {
		...
		finalCredSpecMapDataLength                     int
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			credspecRes := &CredentialSpecResource{
				...
				domainlessGMSATask:             tc.domainlessGMSATask,
			}
			...
			if tc.expectedErrorString != "" {
				...
				assert.Equal(t, tc.finalCredSpecMapDataLength, len(credspecRes.CredSpecMap))
			}
		})
	}
}
