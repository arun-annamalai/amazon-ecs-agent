		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectCreateContainerExec {
				execCfg := types.ExecConfig{
					...
					Detach: true,
					Cmd:    []string{"C:\\Program Files\\Amazon\\SSM\\amazon-ssm-agent.exe"},
				}
			...
			if test.expectedError != nil {
				...
				if !test.execEnabled {
					...
					assert.Equal(t, "C:\\Program Files\\Amazon\\SSM\\amazon-ssm-agent.exe", execMD.CMD)
...
func TestIdempotentStartAgent(t *testing.T) {
	...
	var (
		...
		testCmd    = "C:\\Program Files\\Amazon\\SSM\\amazon-ssm-agent.exe"
	)
	testUUID := "test-uid"
	defer func() {
		newUUID = uuid.New
