	...
	if !validRoleType(roleType) {
		...
		if roleType == credentials.ExecutionRoleType {
			...
			if task.RequiresDomainlessCredentialSpecResource() {
				err = task.SetGMSAExecutionRoleCredentials(iamRoleCredentials)
				if err != nil {
					seelog.Errorf("Unable to SetGMSAExecutionRoleCredentials for task, err: %v messageId: %s", err, messageId)
					return fmt.Errorf("unable to SetGMSAExecutionRoleCredentials %v", err)
				}
			}
		}
	}
